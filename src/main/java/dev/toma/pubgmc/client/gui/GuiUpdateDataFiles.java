...
public class GuiUpdateDataFiles extends GuiWidgets {
    ...
    public void init() {
        ...
        for (int i = 0; i < entries.size(); i++) {
            ...
            ITextComponent widget = new TextComponentTranslation("label.data.mismatched", entryName);
            widget.getStyle().setColor(color);
            CheckboxWidget checkbox = new CheckboxWidget(5, 25 + i * 20, width - 10, 20, widget.getFormattedText(), (state, x, y, clickedCheckbox) -> {
                checkboxStateChanged(state, x, y, clickedCheckbox, identifier);
            });
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        String headerText
        fontRenderer.drawString(TextFormatting.BOLD + HEADER.getFormattedText(), )
        drawWidgets(mc, mouseX, mouseY, partialTicks);
    }

    private void checkboxStateChanged(boolean selected, int x, int y, CheckboxWidget widget, ResourceLocation identifier) {
        if (selected) {
