...
public abstract class EntityVehicle extends EntityDriveable {
    ...
    public boolean canEntityBoardVehicle(SeatPart seat, EntityLivingBase entity) {
        return super.canEntityBoardVehicle(seat, entity) && !this.isBurned() && this.getHealth() > 0;
    }
    ...
    protected void explode() {
        if (this.world.isRemote)
            return;
        boolean doMobGriefing = ForgeEventFactory.getMobGriefingEvent(this.world, this); // TODO maybe add to separate config
    ...
    private void handleDestroyedTick() {
        // TODO bomb after 5 seconds, make destoryed vehicle invulnerble
        if (!this.isDestroyed() || this.isBurned())
            return;
        this.killEngine();
        if (++this.timeOnFire > this.getTicksToExplode()) { // TODO maybe change this or not
            this.explode();
        }
    }
}
