		...
		switch(args[1])
		{
			case "backpackLevel":
			{
				if(args.length > 1)
				{
					...
					else if(feedback) sendFeedback(sender, player.getName() + " has backpack level = " + data.getBackpackLevel());
				}
			...
			case "boost":
			{
				if(args.length > 1)
				{
					if(args.length > 2 && PUBGMCUtil.isValidNumber(args[2]))
					{
						...
						else if(feedback) sendFeedback(sender, TextFormatting.RED + "Number must be in range 0-100!");
					}
			...
			case "nightVision":
			{
				if(args.length > 1)
				{
					if(args.length > 2 && (args[2].equalsIgnoreCase("false") || args[2].equalsIgnoreCase("true")))
					{
						...
						if(feedback) sendFeedback(sender, "Updated state of night vision of " + player.getName() + " to " + b);
					}
